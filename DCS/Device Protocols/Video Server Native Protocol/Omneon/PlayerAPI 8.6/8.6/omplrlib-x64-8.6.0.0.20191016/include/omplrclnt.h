...
// for each clip added. The deleted callback will be called for each clip
// deleted. Register 0 as the callback function to disable the callback.
// The callbacks execute in a thread created when OmPlrClipRegisterCallbacks(),
// OmPlrClipRegisterWriteCallbacks() or OmPlrRegisterChangeCallback()
// is called.
...
// had been opened for write (regardless of write activity).
// Register 0 as the callback function to disable the callback.
// The callbacks execute in the same thread as the clip added and deleted
// callbacks. See OmPlrClipRegisterCallbacks().
...
// Use either a null or empty string when not specifying a match string.
//
...
OMPLRLIB_C_API OmPlrError OmPlrAttach3A(
    OmPlrHandle plrHandle,           // handle returned from OmPlrOpen()
    const char *pClipName,           // name of the clip
    uint clipIn,                     // clip in point (inclusive)
    uint clipOut,                    // clip out point (exclusive)
    ...
    const char *pTrackMatch,         // optional track match string
    OmPlrVideoFrameConvert vfc,      // video frame conversion mode
    uint afd,                        // afd, coded as in section 9 of SMPTE 2016-1
    uint secAudMixSel,               // secondary audio mix select
    OmPlrClipHandle *pClipHandle);   // returned handle to attached clip

OMPLRLIB_C_API OmPlrError OmPlrAttach3W(
    OmPlrHandle plrHandle,           // handle returned from OmPlrOpen()
    const wchar_t *pClipName,        // name of the clip
    uint clipIn,                     // clip in point (inclusive)
    uint clipOut,                    // clip out point (exclusive)
...
#ifdef _UNICODE
#define OmPlrAttach3 OmPlrAttach3W
#else
#define OmPlrAttach3 OmPlrAttach3A
#endif
...
OMPLRLIB_C_API OmPlrError OmPlrAttach4A(
    OmPlrHandle plrHandle,           // handle returned from OmPlrOpen()
    const char *pClipName,           // name of the clip
    uint clipIn,                     // clip in point (inclusive)
    uint clipOut,                    // clip out point (exclusive)
    ...
    uint afd,                        // afd, coded as in section 9 of SMPTE 2016-1
    uint secAudMixSel,               // secondary audio mix select
    const char *pSubtitleFileList,   // optional caption/subtitle file list
    OmPlrClipHandle *pClipHandle);   // returned handle to attached clip

OMPLRLIB_C_API OmPlrError OmPlrAttach4W(
    OmPlrHandle plrHandle,           // handle returned from OmPlrOpen()
    const wchar_t *pClipName,        // name of the clip
    uint clipIn,                     // clip in point (inclusive)
    uint clipOut,                    // clip out point (exclusive)
    ...
    uint afd,                        // afd, coded as in section 9 of SMPTE 2016-1
    uint secAudMixSel,               // secondary audio mix select
    const wchar_t *pSubtitleFileList,// optional caption/subtitle file list
    OmPlrClipHandle *pClipHandle);   // returned handle to attached clip

#ifdef _UNICODE
#define OmPlrAttach4 OmPlrAttach4W
#else
#define OmPlrAttach4 OmPlrAttach4A
#endif
...
// Adds options to OmPleGetApp
OMPLRLIB_C_API OmPlrError OmPlrGetApp1(
    OmPlrHandle plrHandle,  // handle returned from OmPlrOpen()
    OmPlrApp *pPlayerApp,   // Returned application type
    uint *options);         // See OmPlrAppOptions in omplrdefs.h 
...
// Returns clip cache full name information about attached clips.
// Set clipNameSize to the size of clipName buffer in characters including
// the terminating 0 (not size in bytes)
OMPLRLIB_C_API OmPlrError OmPlrGetClipCacheFullNameA(
    OmPlrHandle plrHandle,     // handle returned from OmPlrOpen()
    OmPlrClipHandle clipHandle,// clip handle
    char *pClipName,           // buffer to store the returned name of the clip cache
    uint clipNameSize);        // size of the above buffer

OMPLRLIB_C_API OmPlrError OmPlrGetClipCacheFullNameW(
    OmPlrHandle plrHandle,     // handle returned from OmPlrOpen()
    OmPlrClipHandle clipHandle,// clip handle
    wchar_t *pClipName,        // buffer to store the returned name of the clip cache
    uint clipNameSize);        // size of the above buffer

#ifdef UNICODE
#define OmPlrGetClipCacheFullName OmPlrGetClipCacheFullNameW
#else
#define OmPlrGetClipCacheFullName OmPlrGetClipCacheFullNameA
#endif

// Returns clip cache name information about attached clips.
// Set clipNameSize to the size of clipName buffer in characters including
// the terminating 0 (not size in bytes)
OMPLRLIB_C_API OmPlrError OmPlrGetClipCacheNameA(
    OmPlrHandle plrHandle,     // handle returned from OmPlrOpen()
    OmPlrClipHandle clipHandle,// clip handle
    char *pClipName,           // buffer to store the returned name of the clip cache
    uint clipNameSize);        // size of the above buffer

OMPLRLIB_C_API OmPlrError OmPlrGetClipCacheNameW(
    OmPlrHandle plrHandle,     // handle returned from OmPlrOpen()
    OmPlrClipHandle clipHandle,// clip handle
    wchar_t *pClipName,        // buffer to store the returned name of the clip cache
    uint clipNameSize);        // size of the above buffer
...
#ifdef UNICODE
#define OmPlrGetClipCacheName OmPlrGetClipCacheNameW
#else
#define OmPlrGetClipCacheName OmPlrGetClipCacheNameA
#endif

// Returns clip cache path information (does not include clip name)
// Set clipPathSize to the size of clipPath buffer in characters including
// the terminating 0 (not size in bytes)
OMPLRLIB_C_API OmPlrError OmPlrGetClipCachePathA(
    OmPlrHandle plrHandle,     // handle returned from OmPlrOpen()
    OmPlrClipHandle clipHandle,// clip handle
    char *pClipPath,           // buffer to store the returned path of the clip cache
    uint clipPathSize);        // size of the above buffer

OMPLRLIB_C_API OmPlrError OmPlrGetClipCachePathW(
    OmPlrHandle plrHandle,     // handle returned from OmPlrOpen()
    OmPlrClipHandle clipHandle,// clip handle
    wchar_t *pClipPath,        // buffer to store the returned path of the clip cache
    uint clipPathSize);        // size of the above buffer

#ifdef UNICODE
#define OmPlrGetClipCachePath OmPlrGetClipCachePathW
#else
#define OmPlrGetClipCachePath OmPlrGetClipCachePathA
#endif
...
OMPLRLIB_C_API OmPlrError OmPlrGetClipData(
    ...
    uint *pClipLen);        // returned duration of clip on timline (in
                            // timeline frame rate)

// Returns information about a clip on the timeline.
// This data can be modified with OmPlrSetClipData() and OmPlrSetClipVfc()
OMPLRLIB_C_API OmPlrError OmPlrGetClipData1(
    OmPlrHandle plrHandle,       // handle returned from OmPlrOpen()
    OmPlrClipHandle clipHandle,  // clip handle
    int *pPos,                   // returned timeline position of start of clip
    uint *pClipIn,               // returned in point of clip
    ...
    uint *pClipLen,              // returned duration of clip on timline (in
                                 // timeline frame rate)
    OmPlrVideoFrameConvert *vfc, // video frame conversion mode as specified in
                                 // omPlrAttach2()
    uint *afd,                   // clip afd as specified in omPlrAttach3()
    uint *secAudMixSel);         // clip secondary audio mix sel as specified
                                 // in omPlrAttach3()

// Returns information about a clip on the timeline.
// This data can be modified with OmPlrSetClipData() and OmPlrSetClipVfc()
OMPLRLIB_C_API OmPlrError OmPlrGetClipData2(
    OmPlrHandle plrHandle,       // handle returned from OmPlrOpen()
    OmPlrClipHandle clipHandle,  // clip handle
    int *pPos,                   // returned timeline position of start of clip
    uint *pClipIn,               // returned in point of clip
    ...
    uint *secAudMixSel,          // clip secondary audio mix sel as specified
                                 // in omPlrAttach3()
    bool *isCached);             // true if non-local clip is cached locally.
...
// Another newer version of OmPlrGetPlayerStatus. This version adds type field.
// See omplrdefs.h for differences between OmPlrStatus3 and OmPlrStatus4.
OMPLRLIB_C_API OmPlrError OmPlrGetPlayerStatus4W(
    OmPlrHandle plrHandle,   // handle returned from OmPlrOpen()
    OmPlrStatus4W *pPlrStatus);// pointer to user supplied OmPlrStatus struct

OMPLRLIB_C_API OmPlrError OmPlrGetPlayerStatus4A(
    OmPlrHandle plrHandle,   // handle returned from OmPlrOpen()
    OmPlrStatus4A *pPlrStatus);// pointer to user supplied OmPlrStatus struct

#ifdef UNICODE
#define OmPlrGetPlayerStatus4 OmPlrGetPlayerStatus4W
#else
#define OmPlrGetPlayerStatus4 OmPlrGetPlayerStatus4A
#endif

// Another newer version of OmPlrGetPlayerStatus. This version adds currClipIsCached
// field. See omplrdefs.h for differences between OmPlrStatus4 and OmPlrStatus5.
OMPLRLIB_C_API OmPlrError OmPlrGetPlayerStatus5W(
    OmPlrHandle plrHandle,   // handle returned from OmPlrOpen()
    OmPlrStatus5W *pPlrStatus);// pointer to user supplied OmPlrStatus struct

OMPLRLIB_C_API OmPlrError OmPlrGetPlayerStatus5A(
    OmPlrHandle plrHandle,   // handle returned from OmPlrOpen()
    OmPlrStatus5A *pPlrStatus);// pointer to user supplied OmPlrStatus struct

#ifdef UNICODE
#define OmPlrGetPlayerStatus5 OmPlrGetPlayerStatus5W
#else
#define OmPlrGetPlayerStatus5 OmPlrGetPlayerStatus5A
#endif
...
// Register callback for player changes. The provided callback will be
// called with the provided param when player state or player timeline
// changes. The intent of this callback is to limit the need to poll with
// OmPlrGetPlayerStatus(). Typical usage of this callback should be to
// simply post a semaphore or message to another thread that calls
// OmPlrGetPlayerStatus(), etc. to determine the changes.
// The set of player changes used to trigger this callback includes:
//   state (omPlrState), rate (only when rate changes to or from 1.0),
//   timeline minPos, timeline maxPos, timeline loop min, timeline loop max,
//   playEnabled, recordEnabled, portDown, recordBlackCount (limited to one
//   callback per record session), clip attached, clip detached,
//   clip in point / out point changes.
//
// Register 0 as the callback function to disable the callback.
// The callback executes in the same thread as the clip change callback
// thread, see OmPlrClipRegisterCallbacks().
OMPLRLIB_C_API OmPlrError OmPlrRegisterChangeCallback(
    OmPlrHandle plrHandle,                  // handle returned from OmPlrOpen()
    void (*pPlrChangedCallback)(void *param), // add callback
    void *param);                             // add parameter


// Attaches a locally cached file to a remote clip attached to the timeline
OMPLRLIB_C_API OmPlrError OmPlrSetClipCacheA(
    OmPlrHandle plrHandle,          // handle returned from OmPlrOpen()
    OmPlrClipHandle clipHandle,     // handle of clip to modify
    const char* pClipCacheName);    // name of cached file

// Attaches a locally cached file to a remote clip attached to the timeline
OMPLRLIB_C_API OmPlrError OmPlrSetClipCacheW(
    OmPlrHandle plrHandle,          // handle returned from OmPlrOpen()
    OmPlrClipHandle clipHandle,     // handle of clip to modify
    const wchar_t* pClipCacheName); // name of cached file

#ifdef _UNICODE
#define OmPlrSetClipCache OmPlrSetClipCacheW
#else
#define OmPlrSetClipCache OmPlrSetClipCacheA
#endif
...
// Modifies vfc of a clip on the timeline.
// Use omPlrVfcUnknown to remove previous setting
OMPLRLIB_C_API OmPlrError OmPlrSetClipVfc(
    OmPlrHandle plrHandle,       // handle returned from OmPlrOpen()
    OmPlrClipHandle clipHandle,  // handle of clip to modify
    OmPlrVideoFrameConvert vfc); // video frame conversion mode
...
// Binds the network stream input specified by a set of parameters (local
// interface, stream address, port, stream #, program #) to the specified
// player.
OMPLRLIB_C_API OmPlrError OmPlrStrmInputSetA(
    OmPlrHandle plrHandle,            // handle returned from OmPlrOpen()
    const char *pLocalInterface,      // IP address of local network interface
    const char *pSourceAddress,       // IP address of network stream source
    const char *pMulticastAddress,    // IP address of multicast group
    uint portNum,                     // network port number
    uint programNum);                 // transport stream program_number

OMPLRLIB_C_API OmPlrError OmPlrStrmInputSetW(
    OmPlrHandle plrHandle,            // handle returned from OmPlrOpen()
    const wchar_t *pLocalInterface,   // IP address of local network interface
    const wchar_t *pSourceAddress,    // IP address of network stream source
    const wchar_t *pMulticastAddress, // IP address of multicast group
    uint portNum,                     // network port number
    uint programNum);                 // transport stream program_number

#ifdef UNICODE
#define OmPlrStrmInputSet OmPlrStrmInputSetW
#else
#define OmPlrStrmInputSet OmPlrStrmInputSetA
#endif


// Sets the redundant network stream input specified by a set of parameters (local
// interface, stream address, port, stream #, program #) for the specified
// player.
OMPLRLIB_C_API OmPlrError OmPlrStrmInputRedundantSetA(
    OmPlrHandle plrHandle,            // handle returned from OmPlrOpen()
    const char *pLocalInterface,      // IP address of local network interface
    const char *pSourceAddress,       // IP address of network stream source
    const char *pMulticastAddress,    // IP address of multicast group
...
OMPLRLIB_C_API OmPlrError OmPlrStrmInputRedundantSetW(
    OmPlrHandle plrHandle,            // handle returned from OmPlrOpen()
    const wchar_t *pLocalInterface,   // IP address of local network interface
    const wchar_t *pSourceAddress,    // IP address of network stream source
    const wchar_t *pMulticastAddress, // IP address of multicast group
...
#ifdef UNICODE
#define OmPlrStrmInputRedundantSet OmPlrStrmInputRedundantSetW
#else
#define OmPlrStrmInputRedundantSet OmPlrStrmInputRedundantSetA
#endif


// Queries which network stream input is currently bound to the specified
// player.
OMPLRLIB_C_API OmPlrError OmPlrStrmInputGetA(
    OmPlrHandle plrHandle,          // handle returned from OmPlrOpen()
    char *pLocalInterface,          // IP address of local network interface
    uint localInterfaceSize,        // size (in bytes) of pLocalInterface buffer
    char *pSourceAddress,           // IP address of network stream source
    uint streamAddressSize,         // size (in bytes) of pSourceAddress buffer
    char *pMulticastAddress,        // IP address of multicast group
    uint multicastAddressSize,      // size (in bytes) of pMulticastAddress buffer
    uint *portNum,                  // network port number
    uint *programNum);              // transport stream program_number

OMPLRLIB_C_API OmPlrError OmPlrStrmInputGetW(
    OmPlrHandle plrHandle,          // handle returned from OmPlrOpen()
    wchar_t *pLocalInterface,       // IP address of local network interface
    uint localInterfaceSize,        // size (in bytes) of pLocalInterface buffer
    wchar_t *pSourceAddress,        // IP address of network stream source
    uint streamAddressSize,         // size (in bytes) of pSourceAddress buffer
    wchar_t *pMulticastAddress,     // IP address of multicast group
    uint multicastAddressSize,      // size (in bytes) of pMulticastAddress buffer
    uint *portNum,                  // network port number
    uint *programNum);              // transport stream program_number

#ifdef UNICODE
#define OmPlrStrmInputGet OmPlrStrmInputGetW
#else
#define OmPlrStrmInputGet OmPlrStrmInputGetA
#endif


// Queries which redundant network stream input is currently bound to the specified
// player.
OMPLRLIB_C_API OmPlrError OmPlrStrmInputRedundantGetA(
    OmPlrHandle plrHandle,          // handle returned from OmPlrOpen()
    char *pLocalInterface,          // IP address of local network interface
    uint localInterfaceSize,        // size (in bytes) of pLocalInterface buffer
    char *pSourceAddress,           // IP address of network stream source
...
OMPLRLIB_C_API OmPlrError OmPlrStrmInputRedundantGetW(
    OmPlrHandle plrHandle,          // handle returned from OmPlrOpen()
    wchar_t *pLocalInterface,       // IP address of local network interface
    uint localInterfaceSize,        // size (in bytes) of pLocalInterface buffer
    wchar_t *pSourceAddress,        // IP address of network stream source
...
#ifdef UNICODE
#define OmPlrStrmInputRedundantGet OmPlrStrmInputRedundantGetW
#else
#define OmPlrStrmInputRedundantGet OmPlrStrmInputRedundantGetA
#endif
