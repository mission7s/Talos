...
struct OmTcData {
    ...
    // Binary group 8:
	uint binaryGroup8 : 4;
	uint binaryGroup81 : 4;
	uint binaryGroup82 : 4;
	uint binaryGroup83 : 4;
	uint binaryGroup84 : 4;
	uint binaryGroup85 : 4;
	uint binaryGroup86 : 4;
	uint binaryGroup87 : 4;
	uint binaryGroup88 : 4;
	uint binaryGroup89 : 4;
    ...
    bool GetTime(char* time, OmFieldRate fieldRate) const {
        if (IsInvalid()){
            sprintf(time, "Invalid");
            return false;
        }
        sprintf(time, "%02d:%02d:%02d:%02d:%d",
                Hours(), Minutes(), Seconds(), Frames(), Field(fieldRate));
        return true;
    }
