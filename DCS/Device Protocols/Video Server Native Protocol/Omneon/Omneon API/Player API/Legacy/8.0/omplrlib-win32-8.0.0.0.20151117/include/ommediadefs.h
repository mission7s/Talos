...
enum { omMediaNumTypes = 28 /**< Number of types in OmMediaType (including unused ones) */ };
...
enum OmStreamType {
    ...
    omStreamSoftel      =24,    ///< Stream for sending softel caption data
...
struct MediaSummaryName {
    static const char* get(OmMediaType);                 ///< Returns a string representation of an OnMediaType.
    static const char* get(OmStreamType);                ///< Returns a string representation of an OnStreamType.
    static const char* get(OmVideoSampleRatio);          ///< Returns a string representation of an OnVideoSampleRatio.
    static const char* get(OmVideoAspectRatio);          ///< Returns a string representation of an OnVideoAspectRatio.
    static const char* get(OmFrameRate);                 ///< Returns a string representation of an OnFrameRate.
    static const char* get(OmFrameStruct);

    static OmMediaType        getMediaType(const char*); ///< Returns an OmMediaType that matches the given string, but returns “OmMediaUnknown” if the string is not recognized.
    static OmVideoSampleRatio getVsr(const char*);       ///< Returns an OmVideoSampleRatio that matches the given string, but returns “OmVideoSampleNone” if the string is not recognized.
    static OmVideoAspectRatio getVar(const char*);       ///< Returns an OmVideoAspectRatio that matches the given string, but returns “OmVideoAspectNone” if the string is not recognized.
    static OmMediaFileType    getFileType(const char*);  ///< Returns an OmMediaFileType that matches the given string, but returns omMediaFileTypeUnknown if the string is not recognized.
    static OmFrameRate        getFrameRate(const char*); ///< Returns an OmFrameRate that matches the given string, but returns “OmFrameRateInvalid” if the string is not recognized.
};
#endif
#endif
