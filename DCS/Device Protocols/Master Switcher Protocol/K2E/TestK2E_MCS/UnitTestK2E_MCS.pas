...
type
  TForm1 = class(TForm)
    ...
    procedure btnOutLogoPSTClick(Sender: TObject);
...
procedure TForm1.btnOutLogoPSTClick(Sender: TObject);
var
  R: Integer;
  LogoMask1, LogoMask2, LogoMask3, LogoMask4: Boolean;
  Logo1, Logo2, Logo3, Logo4: Boolean;
begin
  ...
  if (R = D_OK) then
  begin
    mmLog.Lines.Add(Format('OutLogoToPST success. Logo1 = %s, Logo2 = %s, Logo3 = %s, Logo4 = %s', [BoolToStr(Logo1, True), BoolToStr(Logo2, True), BoolToStr(Logo3, True), BoolToStr(Logo4, True)]));
  end
  else
    mmLog.Lines.Add(Format('[Error] OutLogoToPST Fail. errorcode = %d', [R]));
end;
