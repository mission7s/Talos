...
begin
  ...
  R := DCSSysInitialize(8012, 8011);
  if (R <> D_OK) then
    mmDesc.Lines.Add(Format('DCS connection system initialization failed. [Errorcode=%d, Sys in port=%d, Sys out port=%d]', [R, 8012, 8011]));
end;
...
begin
  ...
  R := DCSSysFinalize;
  if (R <> D_OK) then
    mmDesc.Lines.Add(Format('DCS connection system finalization failed. [Errorcode=%d]', [R]));
end;
