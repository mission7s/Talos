...
begin
  ...
  R := FQuartz.SetXptMessage(DstIndex, DstLevel, SrcIndex);

  if (R = D_OK) then
  begin
    mmDesc.Lines.Add(Format('SetXpt Message Success, Dst = %d, Dst level = %d, Src = %d', [DstIndex, DstLevel, SrcIndex]));
  end
  else
  begin
...
procedure TForm12.btnTestConnectedClick(Sender: TObject);
var
  R: Integer;
  Conn: Boolean;
begin
...
begin
  DstIndex := StrToIntDef(edtDstIndex.Text, 0);
  DstLevel := StrToIntDef(edtLvlIndex.Text, 0);
  R := FQuartz.InterrogateRoute(DstIndex, DstLevel, SrcIndex);
