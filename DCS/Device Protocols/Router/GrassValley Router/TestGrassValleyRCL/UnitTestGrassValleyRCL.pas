...
begin
  ...
  R := FGrassValleyRCL.TakebyLevelIndex(DstEntry, LvlIndex, SrcEntry);

  if (R = D_OK) then
  begin
    mmDesc.Lines.Add(Format('TakebyLevelIndex Success, Dst = %d.%d, Dst level = %d, Src = %d.%d', [DstEntry.AreaIndex, DstEntry.Index, LvlIndex, SrcEntry.AreaIndex, SrcEntry.Index]));
  end
  else
  begin
