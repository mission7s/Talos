...
//    procedure GetAncTimecode(TC: PByte; dbb1: Cardinal; vitcA, vitcB: Cardinal);
...
//
    ...
    function ToString(FrameRate: TOmFrameRate): String; overload;
    // Note: field rate alone does not contain enough information to
    // properly render a timecode string.
    function ToString(FieldRate: TOmFieldRate; nFields: Cardinal = 2): String; overload;
  ...
  TOmTimecode = record
    OmTcData: TOmTcData;
    procedure OmTimecode; overload;
    procedure OmTimecode(Val: TOmTcData); overload;
  end;
...
function TOmTcData.ToString(FieldRate: TOmFieldRate; nFields: Cardinal = 2): String;
var
  FrameRate: TOmFrameRate;
begin
  case FieldRate of
    omFldRate24Hz:
      FrameRate := omFrmRate24Hz;
    omFldRate50Hz:
    begin
      if (nFields = 1) then
        FrameRate := omFrmRate50Hz
      else
        FrameRate := omFrmRate25Hz;
    end;
    omFldRate59_94Hz:
    begin
      if (nFields = 1) then
        FrameRate := omFrmRate59_94Hz
      else
        FrameRate := omFrmRate29_97Hz;
    end;
    omFldRate60Hz:
    begin
      if (nFields = 1) then
        FrameRate := omFrmRate60Hz
      else
        FrameRate := omFrmRate30Hz;
    end;
    omFldRateInvalid:
      FrameRate := omFrmRateInvalid;
    else
      FrameRate := omFrmRateInvalid;
  end;

  Result := ToString(FrameRate);
end;
...
procedure TOmTimecode.OmTimecode;
begin
  FillChar(OmTcData.Data, SizeOf(OmTcData.Data), #0);
end;
