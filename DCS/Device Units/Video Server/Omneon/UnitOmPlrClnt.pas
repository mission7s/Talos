...
uses UnitOmPlrDefs, UnitOmClipFileDefs, UnitOmMediaDefs, UnitOmTcData;
...
function OmPlrClipGetTrackUserDataW(
    ...
    const pKey: PAnsiChar;          // name of user data key
...
function OmPlrClipGetTrackUserDataAndKeyW(
    plrHandle: TOmPlrHandle;        // handle returned from OmPlrOpen()
    const pClipName: PChar;         // name of the clip
    trackNum: Integer;              // track number
    dataIdex: Cardinal;             // n (for n'th key/data pair) (0 - n-1)
    pKey: PAnsiChar;                // name of user data key
    keySize: Cardinal;              // size (in bytes) of the above buffer
    data: PAnsiChar;                // returned user data
    dataSize: Cardinal;             // size (in bytes) of the above buffer
    out retDataSize: Cardinal       // returned user data size
    ): TOmPlrError; cdecl; external omplrlib name 'OmPlrClipGetTrackUserDataAndKeyW';

function OmPlrClipGetTrackUserDataAndKeyA(
    plrHandle: TOmPlrHandle;        // handle returned from OmPlrOpen()
    const pClipName: PAnsiChar;     // name of the clip
    trackNum: Integer;              // track number
    dataIdex: Cardinal;             // n (for n'th key/data pair) (0 - n-1)
    ...
    dataSize: Cardinal;             // size (in bytes) of the above buffer
    out retDataSize: Cardinal       // returned user data size
    ): TOmPlrError; cdecl; external omplrlib name 'OmPlrClipGetTrackUserDataAndKeyA';

function OmPlrClipGetTrackUserDataAndKey(
    plrHandle: TOmPlrHandle;        // handle returned from OmPlrOpen()
    const pClipName: TOmPlrChar;     // name of the clip
    trackNum: Integer;              // track number
    dataIdex: Cardinal;             // n (for n'th key/data pair) (0 - n-1)
...
// Note, the returned data in *pData is not null terminated. *pRetDataSize
// is set to the size of the returned data in *pData.
function OmPlrClipGetUserDataW(
    plrHandle: TOmPlrHandle;        // handle returned from OmPlrOpen()
    const pClipName: PChar;         // name of the clip
    const pKey: PAnsiChar;          // name of user data key
    pData: PAnsiChar;               // returned user data
    dataSize: Cardinal;             // size (in bytes) of the above buffer
    out pRetDataSize: Cardinal      // size (in bytes) of the returned user data
    ): TOmPlrError; cdecl; external omplrlib name 'OmPlrClipGetUserDataW';

function OmPlrClipGetUserDataA(
    plrHandle: TOmPlrHandle;        // handle returned from OmPlrOpen()
    const pClipName: PAnsiChar;     // name of the clip
    const pKey: PAnsiChar;          // name of user data key
    pData: PAnsiChar;               // returned user data
    dataSize: Cardinal;             // size (in bytes) of the above buffer
    out pRetDataSize: Cardinal      // size (in bytes) of the returned user data
    ): TOmPlrError; cdecl; external omplrlib name 'OmPlrClipGetUserDataA';

function OmPlrClipGetUserData(
    plrHandle: TOmPlrHandle;        // handle returned from OmPlrOpen()
    const pClipName: TOmPlrChar;    // name of the clip
    const pKey: PAnsiChar;          // name of user data key
    pData: PAnsiChar;               // returned user data
...
//--- Need add more functions
//--- OmPlrClipGetUserDataAndKey ~ OmPlrClipSetDefaultInOut
...
function OmPlrClipWhereRecordingW(
    plrHandle: TOmPlrHandle;      // handle returned from OmPlrOpen()
    pClipName: PChar;             // name of the clip
    pSerialNumber: PAnsiChar;     // buffer to store returned serial number
    serialNumberSize: Cardinal    // size of the above buffer
    ): TOmPlrError; cdecl; external omplrlib name 'OmPlrClipWhereRecordingW';

function OmPlrClipWhereRecordingA(
    plrHandle: TOmPlrHandle;      // handle returned from OmPlrOpen()
    pClipName: PAnsiChar;         // name of the clip
    pSerialNumber: PAnsiChar;     // buffer to store returned serial number
    serialNumberSize: Cardinal    // size of the above buffer
    ): TOmPlrError; cdecl; external omplrlib name 'OmPlrClipWhereRecordingA';

function OmPlrClipWhereRecording(
    plrHandle: TOmPlrHandle;      // handle returned from OmPlrOpen()
    pClipName: TOmPlrChar;        // name of the clip
    pSerialNumber: PAnsiChar;     // buffer to store returned serial number
    serialNumberSize: Cardinal    // size of the above buffer
...
// Use omPlrVfcUnknown to not override the current value.
...
// Yet another version of OmPlrAttach. This one starts with OmPlrAttach2 and
// adds an AFD and secondary audio mix select. These values apply only to the
// attached clip. The AFD overrides any other AFD associated with the clip.
// Use 0 for AFD to not override the current value.
// The secondary audio mix select specifies an index used to look up an audio
// mix mode to convert the main audio to the secondary audio output. Use 0
// to not override the current value.
function OmPlrAttach3W(
    plrHandle: TOmPlrHandle;        // handle returned from OmPlrOpen()
    const pClipName: PChar;         // name of the clip
    clipIn: Cardinal;               // clip in point (inclusive)
    clipOut: Cardinal;              // clip out point (exclusive)
    ...
    const pTrackMatch: PAnsiChar;   // optional track match string
    vfc: TOmPlrVideoFrameConvert;   // video frame conversion mode
    afd: Cardinal;                  // afd, coded as in section 9 of SMPTE 2016-1
    secAudMixSel: Cardinal;         // secondary audio mix select
    out pClipHandle: TOmPlrClipHandle   // returned handle to attached clip
    ): TOmPlrError; cdecl; external omplrlib name 'OmPlrAttach3W';

function OmPlrAttach3A(
    plrHandle: TOmPlrHandle;        // handle returned from OmPlrOpen()
    const pClipName: PAnsiChar;     // name of the clip
    clipIn: Cardinal;               // clip in point (inclusive)
    clipOut: Cardinal;              // clip out point (exclusive)
    ...
    secAudMixSel: Cardinal;         // secondary audio mix select
    out pClipHandle: TOmPlrClipHandle   // returned handle to attached clip
    ): TOmPlrError; cdecl; external omplrlib name 'OmPlrAttach3A';

function OmPlrAttach3(
    plrHandle: TOmPlrHandle;        // handle returned from OmPlrOpen()
    const pClipName: TOmPlrChar;    // name of the clip
    clipIn: Cardinal;               // clip in point (inclusive)
    clipOut: Cardinal;              // clip out point (exclusive)
...
// Yet another version of OmPlrAttach. This one starts with OmPlrAttach3 and
// adds the list of caption or subtitle files to insert during playback.
// This list overrides any other caption or subtitle files associated with
// the clip. The list of files is given as a single string formatted as a
// JSON array, for example:
// "[{\"file\":\"cap_eng.scc\"},{\"file\":\"cap_spa.scc\"}]"
// Filenames can be absolute or relative. If relative then the configured
// subtitle directory is used as a base location. Use an empty string to not
// override the associations set in the player configuration.
function OmPlrAttach4W(
    plrHandle: TOmPlrHandle;        // handle returned from OmPlrOpen()
    const pClipName: PChar;         // name of the clip
    clipIn: Cardinal;               // clip in point (inclusive)
    clipOut: Cardinal;              // clip out point (exclusive)
    ...
    afd: Cardinal;                  // afd, coded as in section 9 of SMPTE 2016-1
    secAudMixSel: Cardinal;         // secondary audio mix select
    const pSubtitleFileList: PChar; // optional caption/subtitle file list
    out pClipHandle: TOmPlrClipHandle   // returned handle to attached clip
    ): TOmPlrError; cdecl; external omplrlib name 'OmPlrAttach4W';

function OmPlrAttach4A(
    plrHandle: TOmPlrHandle;        // handle returned from OmPlrOpen()
    const pClipName: PAnsiChar;     // name of the clip
    clipIn: Cardinal;               // clip in point (inclusive)
    clipOut: Cardinal;              // clip out point (exclusive)
    ...
    afd: Cardinal;                  // afd, coded as in section 9 of SMPTE 2016-1
    secAudMixSel: Cardinal;         // secondary audio mix select
    const pSubtitleFileList: PAnsiChar; // optional caption/subtitle file list
    out pClipHandle: TOmPlrClipHandle   // returned handle to attached clip
    ): TOmPlrError; cdecl; external omplrlib name 'OmPlrAttach4A';

function OmPlrAttach4(
    plrHandle: TOmPlrHandle;        // handle returned from OmPlrOpen()
    const pClipName: TOmPlrChar;    // name of the clip
    clipIn: Cardinal;               // clip in point (inclusive)
    clipOut: Cardinal;              // clip out point (exclusive)
    ...
    afd: Cardinal;                  // afd, coded as in section 9 of SMPTE 2016-1
    secAudMixSel: Cardinal;         // secondary audio mix select
    const pSubtitleFileList: TOmPlrChar; // optional caption/subtitle file list
    out pClipHandle: TOmPlrClipHandle   // returned handle to attached clip
    ): TOmPlrError;
...
//--- Need add more functions
//--- OmPlrEnable ~ OmPlrGetAppClipExtList


// Returns the clip handle of the num'th clip on the timeline.
// Returns 0 if no num'th clips. The first clip is clip 0.
function OmPlrGetClipAtNum(
    plrHandle: TOmPlrHandle;      // handle returned from OmPlrOpen()
    num: Integer;                 // which clip (first is 0)
    out pClipHandle: TOmPlrClipHandle // returned clip handle of num'th clip
    ): TOmPlrError; cdecl; external omplrlib name 'OmPlrGetClipAtNum';

// Returns the handle of the clip located at the given timeline position.
// Returns 0 if no clip at position.
function OmPlrGetClipAtPos(
    plrHandle: TOmPlrHandle;      // handle returned from OmPlrOpen()
    pos: Integer;                 // timeline position
    out pClipHandle: TOmPlrClipHandle;// returned clip handle at specified
                                      // timeline pos
    out pClipStartPos: Integer        // returned timeline pos of the start of the clip
    ): TOmPlrError; cdecl; external omplrlib name 'OmPlrGetClipAtPos';


//--- Need add more functions
//--- OmPlrGetClipData ~ OmPlrGetDropFrame
...
function OmPlrGetPlayerStatus1W(
    plrHandle: TOmPlrHandle;     // handle returned from OmPlrOpen()
    out pPlrStatus: TOmPlrStatus1W // pointer to user supplied OmPlrStatus struct
    ): TOmPlrError; cdecl; external omplrlib name 'OmPlrGetPlayerStatus1W';

function OmPlrGetPlayerStatus1A(
    plrHandle: TOmPlrHandle;     // handle returned from OmPlrOpen()
    out pPlrStatus: TOmPlrStatus1A // pointer to user supplied OmPlrStatus struct
    ): TOmPlrError; cdecl; external omplrlib name 'OmPlrGetPlayerStatus1A';

function OmPlrGetPlayerStatus1(
    plrHandle: TOmPlrHandle;      // handle returned from OmPlrOpen()
    out pPlrStatus: TOmPlrStatus1 // pointer to user supplied OmPlrStatus struct
    ): TOmPlrError;

// player status version 3
function OmPlrGetPlayerStatus2W(
    plrHandle: TOmPlrHandle;     // handle returned from OmPlrOpen()
    out pPlrStatus: TOmPlrStatus2W // pointer to user supplied OmPlrStatus struct
    ): TOmPlrError; cdecl; external omplrlib name 'OmPlrGetPlayerStatus2W';

function OmPlrGetPlayerStatus2A(
    plrHandle: TOmPlrHandle;     // handle returned from OmPlrOpen()
    out pPlrStatus: TOmPlrStatus2A // pointer to user supplied OmPlrStatus struct
    ): TOmPlrError; cdecl; external omplrlib name 'OmPlrGetPlayerStatus2A';

function OmPlrGetPlayerStatus2(
    plrHandle: TOmPlrHandle;      // handle returned from OmPlrOpen()
    out pPlrStatus: TOmPlrStatus2 // pointer to user supplied OmPlrStatus struct
    ): TOmPlrError;

// player status version 4
function OmPlrGetPlayerStatus3W(
    plrHandle: TOmPlrHandle;     // handle returned from OmPlrOpen()
    out pPlrStatus: TOmPlrStatus3W // pointer to user supplied OmPlrStatus struct
    ): TOmPlrError; cdecl; external omplrlib name 'OmPlrGetPlayerStatus3W';

function OmPlrGetPlayerStatus3A(
    plrHandle: TOmPlrHandle;     // handle returned from OmPlrOpen()
    out pPlrStatus: TOmPlrStatus3A // pointer to user supplied OmPlrStatus struct
    ): TOmPlrError; cdecl; external omplrlib name 'OmPlrGetPlayerStatus3A';

function OmPlrGetPlayerStatus3(
    plrHandle: TOmPlrHandle;      // handle returned from OmPlrOpen()
    out pPlrStatus: TOmPlrStatus3 // pointer to user supplied OmPlrStatus struct
    ): TOmPlrError;

// player status version 5
function OmPlrGetPlayerStatus4W(
    plrHandle: TOmPlrHandle;     // handle returned from OmPlrOpen()
    out pPlrStatus: TOmPlrStatus4W // pointer to user supplied OmPlrStatus struct
    ): TOmPlrError; cdecl; external omplrlib name 'OmPlrGetPlayerStatus4W';

function OmPlrGetPlayerStatus4A(
    plrHandle: TOmPlrHandle;     // handle returned from OmPlrOpen()
    out pPlrStatus: TOmPlrStatus4A // pointer to user supplied OmPlrStatus struct
    ): TOmPlrError; cdecl; external omplrlib name 'OmPlrGetPlayerStatus4A';

function OmPlrGetPlayerStatus4(
    plrHandle: TOmPlrHandle;      // handle returned from OmPlrOpen()
    out pPlrStatus: TOmPlrStatus4 // pointer to user supplied OmPlrStatus struct
    ): TOmPlrError;

// player status version 6
function OmPlrGetPlayerStatus5W(
    plrHandle: TOmPlrHandle;     // handle returned from OmPlrOpen()
    out pPlrStatus: TOmPlrStatus5W // pointer to user supplied OmPlrStatus struct
    ): TOmPlrError; cdecl; external omplrlib name 'OmPlrGetPlayerStatus5W';

function OmPlrGetPlayerStatus5A(
    plrHandle: TOmPlrHandle;     // handle returned from OmPlrOpen()
    out pPlrStatus: TOmPlrStatus5A // pointer to user supplied OmPlrStatus struct
    ): TOmPlrError; cdecl; external omplrlib name 'OmPlrGetPlayerStatus5A';

function OmPlrGetPlayerStatus5(
    plrHandle: TOmPlrHandle;      // handle returned from OmPlrOpen()
    out pPlrStatus: TOmPlrStatus5 // pointer to user supplied OmPlrStatus struct
    ): TOmPlrError;


//--- Need add more functions
//--- OmPlrGetPos ~ OmPlrGetPosOfClip
...
//--- Need add more functions
//--- OmPlrGetRecordTime ~ OmPlrGetTcgMode
...
// invalid.
function OmPlrGetTime(
    plrHandle: TOmPlrHandle;      // handle returned from OmPlrOpen()
    out pPos: Integer;            // returned player position
    out pSysFrame: Cardinal;      // returned system frame count
    out pSysFrameFrac: Cardinal;  // returned system fractional frame
    out pRefVitc: TOmTcData;      // returned reference VITC data
    out pTcg: TOmTcData;          // returned timecode generator timecode
    out pVideoTc: TOmTcData       // returned video stream timecode
    ): TOmPlrError; cdecl; external omplrlib name 'OmPlrGetTime';

function OmPlrGetTime1(
    plrHandle: TOmPlrHandle;      // handle returned from OmPlrOpen()
    out pPos: Integer;            // returned player position
    out pSysFrame: Cardinal;      // returned system frame count
    out pSysFrameFrac: Cardinal;  // returned system fractional frame
    ...
    out pTcg: TOmTcData;          // returned timecode generator timecode
    out pVideoTc: TOmTcData;      // returned video stream timecode
    out seconds: Cardinal;        // Director time in seconds
    out milliseconds: Cardinal    // Director time milliseconds portion
    ): TOmPlrError; cdecl; external omplrlib name 'OmPlrGetTime1';


//--- Need add more functions
//--- OmPlrGoToTimecode ~ OmPlrLoop
...
function OmPlrPlayDelay(
    plrHandle: TOmPlrHandle;        // handle returned from OmPlrOpen()
    delay: Cardinal;                // delay value in frames
    playRate: Double = 1.0          // play rate
    ): TOmPlrError; cdecl; external omplrlib name 'OmPlrPlayDelay';

// Returns a players ability to play.
function OmPlrPlayEnabled(
    plrHandle: TOmPlrHandle;        // handle returned from OmPlrOpen()
    out pPlayEnabled: Boolean       // returned true if play is enabled
    ): TOmPlrError; cdecl; external omplrlib name 'OmPlrPlayEnabled';
...
// arrived in.
function OmPlrRecord(
    plrHandle: TOmPlrHandle         // handle returned from OmPlrOpen()
    ): TOmPlrError; cdecl; external omplrlib name 'OmPlrRecord';
...
// The current system frame time (sysFrame) is obtained with OmPlrGetTime().
function OmPlrRecordAt(
    plrHandle: TOmPlrHandle;        // handle returned from OmPlrOpen()
    sysFrame: Cardinal              // system frame time to start record
    ): TOmPlrError; cdecl; external omplrlib name 'OmPlrRecordAt';
...
function OmPlrRecordDelay(
    plrHandle: TOmPlrHandle;        // handle returned from OmPlrOpen()
    delay: Cardinal                 // delay value in frames
    ): TOmPlrError; cdecl; external omplrlib name 'OmPlrRecordDelay';

// Returns a players ability to record.
function OmPlrRecordEnabled(
    plrHandle: TOmPlrHandle;        // handle returned from OmPlrOpen()
    out pPlayEnabled: Boolean       // returned true if record is enabled
    ): TOmPlrError; cdecl; external omplrlib name 'OmPlrRecordEnabled';


//--- Need add more functions
//--- OmPlrRegisterChangeCallback ~ OmPlrSetClipVfc


// Sets the maximum player position. The player position will not
// move to a point at or after this timeline position.
function OmPlrSetMaxPos(
    plrHandle: TOmPlrHandle;     // handle returned from OmPlrOpen()
    maxPos: Integer              // maximum player position (exclusive)
    ): TOmPlrError; cdecl; external omplrlib name 'OmPlrSetMaxPos';
...
function OmPlrSetMinMaxPosToClip(
    plrHandle: TOmPlrHandle;     // handle returned from OmPlrOpen()
    clipHandle: TOmPlrClipHandle // clip handle
    ): TOmPlrError; cdecl; external omplrlib name 'OmPlrSetMinMaxPosToClip';

// Sets the minimum player position. The player position will not
// move to a point before this timeline position.
function OmPlrSetMinPos(
    plrHandle: TOmPlrHandle;     // handle returned from OmPlrOpen()
    minPos: Integer              // minimum player position (inclusive)
    ): TOmPlrError; cdecl; external omplrlib name 'OmPlrSetMinPos';
...
function OmPlrSetMinPosMin(
    plrHandle: TOmPlrHandle      // handle returned from OmPlrOpen()
    ): TOmPlrError; cdecl; external omplrlib name 'OmPlrSetMinPosMin';
...
function OmPlrSetPlayerSwitching(
    plrHandle: TOmPlrHandle;     // handle returned from OmPlrOpen()
    enable: Boolean              // set true to enable switching
    ): TOmPlrError; cdecl; external omplrlib name 'OmPlrSetPlayerSwitching';
...
//--- Need add more functions
//--- OmPlrSetTcgData ~ OmPlrStrmInputGet
...
begin
{$IFDEF UNICODE}
  Result := OmPlrClipGetTrackUserDataAndKeyW(plrHandle, pClipName, trackNum, dataIdex, pKey, keySize, data, dataSize, retDataSize);
{$ELSE}
  Result := OmPlrClipGetTrackUserDataAndKeyA(plrHandle, pClipName, trackNum, dataIdex, pKey, keySize, data, dataSize, retDataSize);
{$ENDIF}
end;
...
begin
{$IFDEF UNICODE}
  Result := OmPlrClipGetUserDataW(plrHandle, pClipName, pKey, pData, dataSize, pRetDataSize);
{$ELSE}
  Result := OmPlrClipGetUserDataA(plrHandle, pClipName, pKey, pData, dataSize, pRetDataSize);
{$ENDIF}
end;
...
begin
{$IFDEF UNICODE}
  Result := OmPlrAttach3W(plrHandle, pClipName, clipIn, clipOut, beforeClipHandle, shiftMode, pTrackMatch, vfc, afd, secAudMixSel, pClipHandle);
{$ELSE}
  Result := OmPlrAttach3A(plrHandle, pClipName, clipIn, clipOut, beforeClipHandle, shiftMode, pTrackMatch, vfc, afd, secAudMixSel, pClipHandle);
{$ENDIF}
end;
...
begin
{$IFDEF UNICODE}
  Result := OmPlrAttach4W(plrHandle, pClipName, clipIn, clipOut, beforeClipHandle, shiftMode, pTrackMatch, vfc, afd, secAudMixSel, pSubtitleFileList, pClipHandle);
{$ELSE}
  Result := OmPlrAttach4A(plrHandle, pClipName, clipIn, clipOut, beforeClipHandle, shiftMode, pTrackMatch, vfc, afd, secAudMixSel, pSubtitleFileList, pClipHandle);
{$ENDIF}
end;
...
begin
{$IFDEF UNICODE}
  Result := OmPlrGetPlayerStatus1W(plrHandle, pPlrStatus);
{$ELSE}
  Result := OmPlrGetPlayerStatus1A(plrHandle, pPlrStatus);
{$ENDIF}
end;
...
begin
{$IFDEF UNICODE}
  Result := OmPlrGetPlayerStatus2W(plrHandle, pPlrStatus);
{$ELSE}
  Result := OmPlrGetPlayerStatus2A(plrHandle, pPlrStatus);
{$ENDIF}
end;
...
begin
{$IFDEF UNICODE}
  Result := OmPlrGetPlayerStatus3W(plrHandle, pPlrStatus);
{$ELSE}
  Result := OmPlrGetPlayerStatus3A(plrHandle, pPlrStatus);
{$ENDIF}
end;
...
begin
{$IFDEF UNICODE}
  Result := OmPlrGetPlayerStatus4W(plrHandle, pPlrStatus);
{$ELSE}
  Result := OmPlrGetPlayerStatus4A(plrHandle, pPlrStatus);
{$ENDIF}
end;
...
begin
{$IFDEF UNICODE}
  Result := OmPlrGetPlayerStatus5W(plrHandle, pPlrStatus);
{$ELSE}
  Result := OmPlrGetPlayerStatus5A(plrHandle, pPlrStatus);
{$ENDIF}
end;
...
begin
{$IFDEF UNICODE}
  Result := OmPlrClipWhereRecordingW(plrHandle, pClipName, pSerialNumber, serialNumberSize);
{$ELSE}
  Result := OmPlrClipWhereRecordingA(plrHandle, pClipName, pSerialNumber, serialNumberSize);
{$ENDIF}
end;
