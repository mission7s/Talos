...
type
  ...
  (
    ...
    omVidFmt2250Line25Hz = $f,          // 2160p25 (internal)
    omVidFmt2250Line29_97Hz = $10,      // 2160p29.97 (internal)
    omVidFmt1125Line25Hz = $11,         // 1080p25 (SMPTE 274M System 9)
    omVidFmt1125Line29_97Hz = $12       // 1080p59.94 (SMPTE 274M System 8)
    // Do not change value of enumerators; add new enumerators at end of list.
  );
...
type
  ...
  (
    ...
    // unused           = 7     // Standard, IEC-15444-1 Annex A codestreams
  ...
  TOmVideoColorSpace = (
    ...
    omVideoColorSpaceBt2100HdrPq = 4, // BT.2100 HDR-PQ color space
    omVideoColorSpaceBt2100HdrHlg = 5 // BT.2100 HDR-HLG color space
  );
  ...
  TOmMediaSummary = record
    MediaType: TOmMediaType;         // As defined in enums
    Vsr: TOmVideoSampleRatio;        // As defined in enums
    Aspect: TOmVideoAspectRatio;     // As defined in enums
    Bitrate: Cardinal;               // bitrate for the track; 0 if its embedded audio
    BitsPerUnit: Cardinal;           // 16 or 24 for audio (unit is a sample); 8 or 10 for supported video formats(unit is a pixel); otherwise 0


    SampleRate: Cardinal;            // Media sample rate * 100; e.g. NTSC video is 2997; 48khz audio is 4800000.

    Channels: Cardinal;              // 1 for video; N for audio union

    constructor Create(const Summary);
    ...
    case Integer of
      ...
      0: // Mpeg
      (
        GopLength: Cardinal;     // Length of the gop structure; usually 15 for MPEG, but is one for other formats

        SubGopLength: Cardinal;  // Length of the sub-gop; usually 3 for IBBPBBP... MPEG video; one for other formats

      );
      // Provides audio information if OmMediaType type is audio.
      1: // Audio
      (
        Format: TOmAudioFormat;  // PCM, Raw, or AC3
        BigEndian: Byte;         // 1 if data is Big Endian, 0 if data is little Endian
        SampleStride: Byte;      // Number of bytes to next sample
      );
      // Provides VBI information if OmMediaType type is VBI.
      2: // Vbi
      (
        LineMask: Cardinal;      // Bit mask indicating lines that are encoded
      );
  end;
  ...
  TOmMediaSummary1 = record
    ...
    Aspect: TOmVideoAspectRatio;     // As defined in enums
    Bitrate: Cardinal;               // Bitrate for the track; 0 if its embedded audio
    BitsPerUnit: Cardinal;           // 16 or 24 for audio (unit is a sample); 8 or 10 for supported video format(unit is a pixel); otherwise 0
    ...
    constructor Create(const Summary1);
    ...
    case Integer of
      ...
      (
        ...
        Width: Word;             // Video width (in pixels)
        Height: Word;            // Video height (in pixels)
      );
      // Provides audio information if OmMediaType type is audio.
      1: // Audio
      (
...
function getRealFrameRate(code: TOmFrameRate): double; inline;
begin
  case code of
    omFrmRate23_976Hz: Result := 24000 / 1001.0;
    omFrmRate24Hz:     Result := 24.0;
    omFrmRate25Hz:     Result := 25.0;
    omFrmRate29_97Hz:  Result := 30000 / 1001.0;
    omFrmRate30Hz:     Result := 30.0;
    omFrmRate50Hz:     Result := 50.0;
    omFrmRate59_94Hz:  Result := 60000 / 1001.0;
    omFrmRate60Hz:     Result := 60.0;
    else Result := 1.0;
  end;
end;

constructor TOmMediaSummary.Create(const Summary);
begin
  MediaType := omMediaUnknown;
...
constructor TOmMediaSummary1.Create(const Summary1);
begin
  MediaType := omMediaUnknown;
  Vsr := omVideoSampleNone;
  Aspect := omVideoAspectNone;
