...
{      vtAscii:
      begin
        StrPLCopy(X^.DstName, DstName);
        StrPLCopy(X^.DstLevelName, DstLevelName);
      end;
      vtUtf8:
      begin
        StrPLCopy(X^.DstUtf8, DstUtf8);
        StrPLCopy(X^.DstLevelUtf8, DstLevelUtf8);
      end; }
      vtNumber:
      begin
        X^.DstIndex := DstIndex;
...
{            vtAscii:
            begin
              StrPLCopy(X^.SrcName, SrcName);
              StrPLCopy(X^.SrcLevelName, SrcLevelName);
            end;
            vtUtf8:
            begin
              StrPLCopy(X^.SrcUtf8, SrcUtf8);
              StrPLCopy(X^.SrcLevelUtf8, SrcLevelUtf8);
            end; }
            vtNumber:
            begin
              X^.SrcIndex := SrcIndex;
