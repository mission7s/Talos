...
type
  ...
  public
    ...
    function SetXptMessage(ADstIndex, ALevelIndex, ASrcIndex: Integer): Integer; overload;
...
begin
  ...
  Buffer := QUARTZ_SOH + ACMD + ADataBuf + QUARTZ_EOT;
...
{  Buffer := GetMemory(Length(ACMD) + ADataSize + 2);
  try
    ...
    Buffer[Length(ACMD) + ADataSize + 1] := Ord(QUARTZ_EOT);
...
begin
  ...
  if (SOHPos >= 0) then
  begin
    EOTPos := Pos(QUARTZ_EOT, FReceivedBuffer);
    if (EOTPos >= 0) then
    begin
...
function TQuartz.SetXptMessage(ADstIndex, ALevelIndex, ASrcIndex: Integer): Integer;
var
  Buffer: AnsiString;
begin
  Result := D_FALSE;
  ...
  Buffer := Format('%s%d,%d', [QUARTZ_LEVELS[ALevelIndex], ADstIndex + 1, ASrcIndex + 1]);
...
begin
  ...
  Buffer := Format('%s%d', [QUARTZ_LEVELS[ALevelIndex], ADstIndex + 1]);

  Result := TransmitCommand('I', Buffer, Length(Buffer));
  if (Result = D_OK) then
  begin
    if (Length(FReceivedData) >= 3) then
    begin
      DivPos := Pos(',', FReceivedData);
      if (DivPos > 0) then
      begin
        ASrcIndex := StrToIntDef(Copy(FReceivedData, DivPos + 1, Length(FReceivedData)), 0);
        if (ASrcIndex > 0) then Dec(ASrcIndex);
      end;
    end;
  end;
end;
