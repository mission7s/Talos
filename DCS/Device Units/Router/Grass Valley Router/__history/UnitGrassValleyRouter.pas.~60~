...
begin
  ...
  Buffer := GVR_SOH + 'R' + SessionIDStr + MessageIDStr + SeqNumberStr + Chr($FF) + Chr($FF) + Chr($FF) + Chr($FF);

  for I := 1 to Length(ACMD) do
    Buffer := Buffer + ACMD[I];

  Buffer := Buffer + Chr($09);

  for I := 1 to ADataSize do
    Buffer := Buffer + ADataBuf[I];

  CheckSum := 0;
  for I := 1 to Length(ACMD) + ADataSize + 16 do
  ...
  Buffer := Buffer + CheckSumStr + GVR_EOT;
...
function TGrassValleyRouter.TakebyLevelIndex(ADstEntry: TFullQualEntry; ASrcEntry: TFullQualEntry; ALevel: SmallInt; ATimeStamp: String = ''): Integer;
var
  Buffer: AnsiString;
begin
  Buffer := ATimeStamp + #9 + GetFullQualIndexString(ADstEntry) + #9 + GetFullQualIndexString(ASrcEntry);
  if (ALevel >=0) then
...
function TGrassValleyRouter.QueryDestinationStatusLevelByIndex(ADstEntry: TFullQualEntry; ALevel: SmallInt; var AProtect: Boolean; var AChopping: Boolean; var ASrcEntry: TFullQualEntry; var AChopEntry: TFullQualEntry): Integer;
var
  Buffer: AnsiString;
begin
  ...
  Buffer := GetFullQualIndexString(ADstEntry) + #9 + Format('%.2x', [ALevel]);
