...
type
  ...
  public
    ...
    function GetMatrix(ADestination: Word; ALevel: Byte; var ASource: Integer): Integer; overload;
  published
...
function TUthaRCP3.GetMatrix(ADestination: Word; ALevel: Byte; var ASource: Integer): Integer;
var
  Buffer: AnsiString;
begin
  Result := D_FALSE;
  ...
  if not (InRange(ALevel, 0, MAX_LEVEL - 1)) then exit;
  ...
  begin
    ...
    if (PAnsiCharToWordR(@FReceivedData[1]) = ADestination) and
       (PAnsiCharToWordR(@FReceivedData[3]) = 1) then
    begin
      ASource := PAnsiCharToWordR(@FReceivedData[5 + SizeOf(Word) * ALevel]);
      Result := D_OK;
    end;
  end;
end;
